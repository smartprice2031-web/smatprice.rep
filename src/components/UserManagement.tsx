...
import { Plus, Trash2, Shield, Store, Search, X, History, User, Calendar, Clock } from 'lucide-react';
...
export default function UserManagement() {
  const { allowedStores, addAllowedStore, removeAllowedStore, accessLogs } = useStore();
  const [activeTab, setActiveTab] = useState<'stores' | 'history'>('stores');
  const [newCnpj, setNewCnpj] = useState('');
  const [newBandeira, setNewBandeira] = useState('Ultra Popular');
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex border-b border-zinc-200 dark:border-zinc-800 px-6">
        <button 
          ...
          className={cn(
            "py-4 px-6 text-xs font-black uppercase tracking-widest flex items-center gap-2 border-b-2 transition-all",
            activeTab === 'stores' 
